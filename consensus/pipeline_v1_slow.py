#!/usr/bin/env python3
"""Consensus email finder v2 — multi-source, dedupe, incremental save."""
...
JUNK = ('example.com','yourdomain','domain.com','test@','@test.','noreply','no-reply','donotreply','do-not-reply','wixpress','sentry.io','wordpress.com','sentry-next.wixpress','wix.com','squarespace','godaddy','@2x.png','@3x.png')
...
def http_get(url, timeout=TIMEOUT):
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout, context=ctx) as r:
            ...
            return r.geturl(), data.decode(cs, errors='replace')
    except Exception:
        return None, None

def extract(text, domain):
    if not text: return []
    par = parent(domain); dlow = domain.lower()
    out = []
...
def src_direct(domain):
    paths = ["/.well-known/security.txt","/security.txt","/security","/security/","/security.html",
             ...
             "/.well-known/dnt-policy.txt","/privacy","/terms"]
    results = []
    for scheme in ("https",):
        for p in paths:
        ...
        if results: break
    # Also try root + scrape its anchors for security/contact links
    root_final, root_body = http_get(f"https://{domain}/", timeout=6)
    if root_body:
        for em, snip in extract(root_body, domain):
            results.append({"email":em,"url":root_final or f"https://{domain}/","snippet":snip,"via":"direct","on_official":True})
    return results

def _scrape_results(url, domain, via):
    final, body = http_get(url, timeout=8)
    if not body: return []
    if via == "ddg":
        link_re = re.compile(r'<a[^>]+class="result__a"[^>]+href="([^"]+)"', re.I)
        urls = []
        for href in link_re.findall(body)[:5]:
            if 'uddg=' in href:
                try:
        ...
        urls = re.findall(r'<li class="b_algo"[^>]*>\s*<h2><a[^>]+href="([^"]+)"', body, re.I)[:5]
    elif via == "google":
        urls = []
    ...
    for u in urls:
        ...
        on_official = host.endswith(par)
        _, ptext = http_get(u, timeout=6)
        if not ptext: continue
        for em, snip in extract(ptext, domain):
            results.append({"email":em,"url":u,"snippet":snip,"via":via,"on_official":on_official})
    return results

def src_ddg(d):    return _scrape_results(f"https://html.duckduckgo.com/html/?q={quote_plus(f'{d} security vulnerability report email')}", d, "ddg")
def src_bing(d):   return _scrape_results(f"https://www.bing.com/search?q={quote_plus(f'{d} security vulnerability report email')}", d, "bing")
def src_google(d): return _scrape_results(f"https://www.google.com/search?q={quote_plus(f'{d} security vulnerability report email')}&hl=en", d, "google")

def process(domain):
    domain = domain.strip().lower()
    if not domain: return None
    findings = []
    for fn in (src_direct, src_ddg, src_bing, src_google):
        try: findings.extend(fn(domain))
    ...
    for f in findings:
        ...
        by[em]["sources"].add(f["via"])
        by[em]["urls"].add(f["url"])
        by[em]["on_official"] = by[em]["on_official"] or f["on_official"]
        if len(by[em]["snippets"]) < 3:
            by[em]["snippets"].append(f["snippet"])
    ranked = []
    for em, d in by.items():
        ...
        # Determine tier
        is_sec_local = any(k in local for k in SEC_LOCALS)
        if is_sec_local and d["on_official"]: tier = "A"
        elif d["on_official"]: tier = "B"
        else: tier = "C"
...
def main():
    ...
    if os.path.exists(out_json + '.partial'):
        try:
            with open(out_json + '.partial') as f:
    ...
    print(f"Scanning {len(todo)} domains ({len(done)} cached) with {workers} workers...")
    t0 = time.time()
    save_every = 25
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for i, fu in enumerate(as_completed(futs)):
            ...
            if (i+1) % 25 == 0 or (i+1)==len(todo):
                ...
                print(f"  [{i+1}/{len(todo)}] {el:.0f}s elapsed, {rate:.1f}/s, ETA {eta:.0f}s, hits {hits}/{len(results)}")
            if (i+1) % save_every == 0:
                with open(out_json + '.partial','w') as f:
                    json.dump(results, f)
    # Final write
    ...
    print(f"DONE. {sum(1 for r in results if r.get('candidates'))}/{len(results)} domains have ≥1 candidate. Total time: {time.time()-t0:.0f}s")
